...
class View:
    def __init__(self, top=None):
        ...
        self.songProgress.place(relx=0.393, rely=0.209,
                                relwidth=0.495, relheight=0.0, height=7)

        self.songTotalDuration = ttk.Label(top)
        self.songTotalDuration.place(
            relx=0.844, rely=0.171, height=19, width=29)
        ...
        self.playList.place(relx=0.0, rely=0.38,
                            relheight=0.532, relwidth=0.999)
        ...
        #self.nextButton.place(relx=0.795, rely=0.285, height=16, width=16)
        ...
        #self._img12 = tk.PhotoImage(file="./icons/next.png")
        # self.nextButton.configure(image=self._img12)
        ...
        self.vol_scale.place(relx=0.015, rely=0.932, relwidth=0.146,
                             relheight=0.0, height=26, bordermode='ignore')
        self.vol_scale.configure(takefocus="")
        self.my_slider = ttk.Scale(top, length=300)
        self.my_slider.place(relx=0.393, rely=0.225, relwidth=0.495,
                             relheight=0.0, height=10, bordermode='ignore')
        self.my_slider.configure(takefocus="")

        self.addSongsToPlayListButton = tk.Button(top)
        self.addSongsToPlayListButton.place(
            relx=0.961, rely=0.323, height=17, width=17)
        ...
        self.deleteSongsFromPlaylistButton.place(
            relx=0.917, rely=0.323, height=18, width=18)
    ...
    def setup_player(self):
        ...
        try:
            ...
            if self.my_player.get_db_status():
                messagebox.showinfo(
                    "Success!", "Connected successfully to the DB")
                self.addFavourite.config(command=self.add_song_to_favourites)
                self.loadFavourite.config(
                    command=self.load_songs_from_favourites)
                self.removeFavourite.config(
                    command=self.remove_song_from_favourites)
            else:
                raise Exception("Sorry! You cannot save or load favourite")
    ...
    def add_song(self):
        ...
        def rcolor(): return random.randint(0, 255)
        red = hex(rcolor())
        green = hex(rcolor())
        blue = hex(rcolor())
        red = red[2:]
    ...
    def play_song(self):
        ...
        try:
            if len(self.sel_song_index_tuple) == 0:
                raise NoSongSelectedError("Plaese select a song to play")
    ...
    def setup_thread(self):
        self.my_Thread = threading.Thread(
            target=self.show_timer, args=(self.song_length,))
        self.isPlaying = True
        self.isThreadRunning = True
        self.my_Thread.start()
        self.my_slider.config(from_=0, to=self.song_length,
                              command=self.drag_song)
    ...
    def show_timer(self, total_sec):
        ...
        while self.curr_sec <= total_sec:
            ...
            if self.isPaused == False:
                ...
                self.my_slider.set(self.curr_sec)
                time.sleep(1)
                self.curr_sec += 1
    ...
    def drag_song(self, pos):
        if (float(pos) == float(self.curr_sec)) == False:
            ...
            self.curr_sec = pos
            if self.curr_sec != self.song_length:
    ...
    def closewindow(self):
        ...
        if result:
            ...
            messagebox.showinfo("Have a good day!",
                                "Thank you for using \"MOUZIKKA\"")
            self.top.destroy()
        ...
        try:
            if len(fav_song_index_tuple) == 0:
                raise NoSongSelectedError(
                    "Please select a song before adding to favourites")
    ...
    def load_songs_from_favourites(self):
        try:
            ...
            if result.find("No songs present") != -1:
                messagebox.showerror("Favourites Empty",
                                     "No song present in your favourites")
                return
            ...
            self.playList.config(fg=mycolor)
            messagebox.showinfo("Favourites loaded!!!",
                                "song loaded successfully in your favourites ")
        except(DatabaseError) as ex2:
    ...
    def remove_song_from_favourites(self):
        ...
        try:
            ...
            if(result.find("Song not present") != 0):
                self.stop_song()
            messagebox.showinfo("Success", result)
            ...
            messagebox.showerror(
                "DB Error!", "Song cannot be removed from favourites")
